...
class Assets
{
    ...
    public function config(array $config)
    {
        ...
        // Set common options
        foreach (['public_dir', 'css_dir', 'js_dir', 'packages_dir', 'pipeline', 'pipeline_dir', 'pipeline_gzip', 'composer_dir'] as $option) {
    ...
    public function add($asset)
    {
        ...
        // Collection
        elseif (isset($this->collections[$asset])) {$this->add($this->collections[$asset]);}
        // JavaScript asset
        elseif (preg_match($this->js_regex, $asset)) {$this->addJs($asset);}
        // CSS asset
        elseif (preg_match($this->css_regex, $asset)) {$this->addCss($asset);}
        return $this;
    }
    /**
    ...
    public function prepend($asset)
    {
        ...
        // Collection
        elseif (isset($this->collections[$asset])) {$this->prepend($this->collections[$asset]);}
        // JavaScript asset
        elseif (preg_match($this->js_regex, $asset)) {$this->prependJs($asset);}
        // CSS asset
        elseif (preg_match($this->css_regex, $asset)) {$this->prependCss($asset);}
        return $this;
    }
    /**
     * Add a CSS asset.
    ...
    public function js($attributes = null)
    {
        ...
        $output = '';
        foreach ($assets as $asset) {
            $output .= '<script src="' . _SRC_URL . $asset . '"' . $attributes . "></script>\n";
        }
    ...
    protected function packLinks(array $links, \Closure $minifier)
    {
        ...
        foreach ($links as $link) {
            ...
            if ($this->fetch_command instanceof \Closure) {
                $content = $this->fetch_command->__invoke($link);
            } else {
                $content = file_get_contents($link, false, stream_context_create($arrContextOptions));
            }

            //echo file_get_contents($link, false, stream_context_create($arrContextOptions));

            if (empty($content)) {
                trigger_error("File empty : " . $originalLink, E_USER_WARNING);
                continue;
            }
