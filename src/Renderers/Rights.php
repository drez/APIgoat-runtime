...
class Rights
{
    ...
    {
        $td = self::getRightsCheckbox($group, $column, $name, 'r', strstr($right,  'r'));
        $td .= self::getRightsCheckbox($group, $column, $name, 'a', strstr($right,  'a'));
        $td .= self::getRightsCheckbox($group, $column, $name, 'w', strstr($right,  'w'));
        $td .= self::getRightsCheckbox($group, $column, $name, 'd', strstr($right,  'd'));
        return tr(td(htmlLink($display, 'Javascript:void(0);', 'class="bld-rights-modules-link"'), "j='chkRights' i='" . $group . $name . "'") . $td);
    }

    static function getRightsCheckbox($group, $column, $name, $right, $checked)
    {
        $check = '';
        if ($checked) {
